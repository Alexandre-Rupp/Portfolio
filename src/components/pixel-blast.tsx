...
export default function PixelBlast({
  ...
  useEffect(() => {
    ...
    const onPointerDown = (event: PointerEvent) => {
      ...
      const clickPos = uniforms.uClickPos.value as THREE.Vector2[];
      const clickTimes = uniforms.uClickTimes.value as Float32Array;
      clickPos[clickIndex].set(fx, fy);
      clickTimes[clickIndex] = uniforms.uTime.value as number;
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      touchTexture.addTouch({ x: fx / w, y: fy / h });
    };
    ...
    const animate = () => {
      ...
      if (liquidEffect) {
        const uniform = liquidEffect.uniforms.get("uTime") as THREE.IUniform<number> | undefined;
        if (uniform) {
          uniform.value = uniforms.uTime.value as number;
        }
      }
  ...
  return (
    <div
      ...
      style={style}
      aria-label="PixelBlast side background"
    />
  );
}
